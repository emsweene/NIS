...
data <- read.table("/project/taki2/NIS/NIS_2007/NIS_2007_Core.ASC",  sep = "\t", header = FALSE)

#########################################################################################################################
## Load in indices for the patients of interest (Acute Ischemic Stroke (AIS), HIV +, and tPA treatement)
#########################################################################################################################

load('5_2007_output.RData')
index_it_5 <- index_it_5_2007
load('433_2007_output.RData')
index_433 <- index_it_109_2007 
load('434_2007_output.RData')
index_434 <- index_it_109_2007
load('436_2007_output.RData')
index_436 <- index_it_109_2007 
load('9910_2007_output.RData')
index_it_9910 <-  index_it_99_2007
...
RACE <- apply(ais_stroke, 2 , function(x) substr(x, start = 476, stop = 477))
...
FEMALE <- apply(ais_stroke, 2 , function(x) substr(x, start = 224, stop = 225))
LOS <- apply(ais_stroke, 2 , function(x) substr(x, start = 252, stop = 256))
Year <- rep(2007, length(ais_stroke))

#########################################################################################################################
## Create and save dataframe 
#########################################################################################################################

data_2007 <- data.frame(hiv, tpa, Other_Diagnosis_1, Other_Diagnosis_2, Other_Diagnosis_3, Other_Diagnosis_4,
...
save(data_2007, file = 'ais_stroke_2007.RData') 